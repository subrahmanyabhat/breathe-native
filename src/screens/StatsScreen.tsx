      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={ss.section}>
          ...
          <View style={ss.subRow}>
            <Text style={[ss.subTxt, { color: th.teal }]}>+{data.sessions.length} sessions</Text>
            <Text style={[ss.subTxt, { color: th.text2 }]}>  · {streak} day streak</Text>
          </View>
        </View>
        ...
        <View style={[ss.card, { marginHorizontal: 20, marginBottom: 14 }]}>
          ...
          <View style={ss.tlRow}>
            {Array.from({ length: TL_SLOTS }, (_, i) => {
              const h = TL_START + i;
              return <View key={i} style={[ss.tlSeg, { backgroundColor: activeHours.has(h) ? th.teal : h === nowHour ? th.teal + '35' : th.text4 }]} />;
            })}
          </View>
        ...
        <View style={[ss.pairRow, { paddingHorizontal: 20, marginBottom: 14 }]}>
          <View style={[ss.pairCard, { backgroundColor: streak > 0 ? 'rgba(232,162,60,0.08)' : th.surf, borderColor: streak > 0 ? 'rgba(232,162,60,0.18)' : th.border }]}>
            <Text style={ss.pairLabel}>STREAK</Text>
            <View style={ss.pairNumRow}>
              <Text style={[ss.pairNum, { color: streak > 0 ? '#e8a23c' : th.text2 }]}>{streak}</Text>
              <Text style={[ss.pairUnit, { color: streak > 0 ? 'rgba(232,162,60,0.55)' : th.label }]}>days</Text>
            </View>
            <Text style={[ss.pairSub, { color: streak > 0 ? 'rgba(232,162,60,0.55)' : th.label }]}>
          ...
          <View style={[ss.pairCard, { backgroundColor: th.surf, borderColor: th.border }]}>
            ...
            <View style={ss.pairNumRow}>
              <Text style={[ss.pairNum, { color: th.text }]}>{best}</Text>
              <Text style={[ss.pairUnit, { color: th.label }]}>days</Text>
            </View>
            <Text style={[ss.pairSub, { color: th.label }]}>personal best</Text>
          </View>
        </View>
            ...
            {days.map((d, i) => {
              ...
              return (
                <View key={d.date} style={[ss.dayRow, { borderBottomWidth: i < days.length - 1 ? 1 : 0, borderBottomColor: th.border }]}>
                  <Text style={[ss.dayLabel, { color: d.isToday ? th.teal : th.text2, fontWeight: d.isToday ? '600' : '400' }]}>{d.label}</Text>
                  <View style={ss.dayBarBg}>
                    <View style={[ss.dayBarFill, { width: `${pct}%` as any, backgroundColor: d.count >= GOAL ? th.teal : d.count > 0 ? th.teal + '60' : 'transparent' }]} />
                  </View>
                  <Text style={[ss.dayCount, { color: d.count > 0 ? th.teal : th.label }]}>{d.count > 0 ? d.count : '·'}</Text>
                </View>
              );
            })}
          </View>
          ...
          <View style={ss.section}>
            ...
            <View style={[ss.card, { marginTop: 10 }]}>
              {TECHNIQUES.filter(t => tc[t.id]).sort((a, b) => (tc[b.id] || 0) - (tc[a.id] || 0)).map((t, i, arr) => {
                ...
                return (
                  <View key={t.id} style={[ss.techRow, { borderBottomWidth: i < arr.length - 1 ? 1 : 0, borderBottomColor: th.border }]}>
        ...
        <View style={ss.section}>
          <View style={[ss.card, { padding: 15 }]}>
            ...
            <View style={ss.goalBarBg}>
              <View style={[ss.goalBarFill, { width: `${Math.min(100, (todaySess.length / GOAL) * 100)}%` as any, backgroundColor: todaySess.length >= GOAL ? '#4dbfae' : th.teal }]} />
            </View>
          </View>
        </View>
        ...
        {data.sessions.length === 0 && (
          <View style={{ alignItems: 'center', paddingVertical: 40 }}>
            ...
            <Text style={{ color: th.label, fontSize: 14, textAlign: 'center' }}>complete your first session{'\n'}to see progress here.</Text>
          </View>
        )}
      </ScrollView>
    </SafeAreaView>
